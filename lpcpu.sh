...
function stop_slabtop() {
	echo "Stopping slabtop."
	kill $ISLABTOP_PID
}
...
# main block, used to log all output
{
    ...
    for IF in /sys/class/net/*; do
        ...
        IF=$(basename $IF)
        [ "$IF" == "lo" ] && continue
        ethtool -S $IF > $LOGDIR/ethtool/ethtool-$IF-stats.before.STDOUT 2> $LOGDIR/ethtool/ethtool-$IF-stats.before.STDERR
    done
    ...
    for IF in /sys/class/net/*; do
        ...
        IF=$(basename $IF)
        [ "$IF" == "lo" ] && continue
        ethtool -S $IF > $LOGDIR/ethtool/ethtool-$IF-stats.after.STDOUT 2> $LOGDIR/ethtool/ethtool-$IF-stats.after.STDERR
    done
    ...
    for IF in /sys/class/net/*; do
        ...
        IF=$(basename $IF)
        [ "$IF" == "lo" ] && continue
	echo 'if [ -x ${NDIFF} -a -e ethtool/ethtool-'"$IF"'-stats.before.STDOUT -a -e ethtool/ethtool-'"$IF"'-stats.after.STDOUT  ]; then ${NDIFF} ethtool/ethtool-'"$IF"'-stats.before.STDOUT ethtool/ethtool-'"$IF"'-stats.after.STDOUT > ethtool/ethtool-'"$IF"'-stats.diff ; fi' >> $LOGDIR/postprocess.sh    
    done
    ...
    lshw > $LOGDIR/lshw.STDOUT 2> $LOGDIR/lshw.STDERR
	...
	cp -aL /etc/os-release $LOGDIR
    fi

    ARCH=`uname -m`
    if [ "$ARCH" == "ppc64" -o "$ARCH" == "ppc64le" ]; then
       if [ -e /proc/device-tree ]; then
           tar -chjf ${LOGDIR}/device_tree.tar.bz2 /proc/device-tree > $LOGDIR/device_tree.STDOUT 2> $LOGDIR/device_tree.STDERR
       fi
